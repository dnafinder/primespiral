...
switch PrimesFamilies{selected}
    ...
    case 'Euler primes (n^2 + n + 41)'
        ...
        case 'Factorial primes'
        % Definition:
        %   Factorial primes are prime numbers of the form n! - 1 or n! + 1.
        %   This family highlights factorial-adjacent primes within 1:t.
        %
        % Subtypes (color-coded):
        %   - "Minus" factorial primes:  n! - 1 is prime
        %   - "Plus"  factorial primes:  n! + 1 is prime
        %
        % Computational idea:
        %   Factorials grow very fast. We generate n! iteratively,
        %   build candidates (n! - 1) and (n! + 1) within range,
        %   test primality, and flag the two subtypes with different colors.

        % -----------------------------------------------------------------
        % First pass: count how many factorial steps can contribute <= t
        % -----------------------------------------------------------------
        f = 1;
        n = 1;
        nmax = 0;
        ...
        % -----------------------------------------------------------------
        % Second pass: generate candidates without growing arrays
        % -----------------------------------------------------------------
        minusCand = zeros(nmax,1);
        plusCand  = zeros(nmax,1);
        km = 0; kp = 0;

        f = 1;
        n = 1;

        % Include the n=1 case: 1! + 1 = 2
        if t >= 2
            kp = kp + 1;
            plusCand(kp) = 2;
        end
        ...
        minusCand = unique(minusCand(1:km));
        plusCand  = unique(plusCand(1:kp));
        ...
        % -----------------------------------------------------------------
        PrimesFlag(minusPrimes) = 2;
        PrimesFlag(plusPrimes)  = 3;

        overlap = intersect(minusPrimes, plusPrimes);
        if ~isempty(overlap)
            PrimesFlag(overlap) = 4;
        end

        primesout = unique([minusPrimes; plusPrimes]);
        ...
        description = {upper('Factorial primes'),...
            'Factorial primes are primes of the form n! - 1 or n! + 1.',...
            'This implementation highlights the two subtypes with different colors.'};
...
nLab = min(numel(H), numel(txt));
legend(H(1:nLab), txt(1:nLab), 'Location','NorthOutside','Units','pixels')
axis square
